---
title: "SnikketX quick-start guide"
subtitle: "How to set up a self-hosted SnikketX instance"
linktitle: "Quick-start"
description: "This guide will help you set up your self-hosted SnikketX instance."
date: 2022-01-25T12:32:02Z
weight: 10
---

{{< lead >}}
Hi, welcome! In this guide we will help you set up your own instance of [SnikketX](/service/). Once it is set up,
you will be able to invite others to join you using the [SnikketX app](/app/) and chat over your own
...
If you don't have a domain name yet, see the FAQ ["Do I need to register a domain name to use SnikketX?"](https://snikket.org/faq/#q-do-i-need-to-register-a-domain-name-to-use-snikket)
for some advice.

**Note:** SnikketX provides a built-in web server that must be accessible on port 80. This guide assumes you are _not_ running any existing
...
Finally, if you can't meet these requirements but want to use SnikketX anyway, check out our [SnikketX Hosting](https://snikket.org/hosting/) service which lets you set up SnikketX with minimal technical knowledge and just a few clicks.

## Get Started

### Step 1: DNS

You need to set up DNS records so that the SnikketX apps can look up and connect to your server.

{{< panel style="info">}}
**Hosting at home?**
If you plan to host your SnikketX instance at home, check with your ISP whether you have a static or dynamic IP address on your home connection.
For advice on setting up SnikketX with a dynamic IP, see ["Can I host SnikketX if I have a dynamic IP address?"](https://snikket.org/faq/#q-can-i-host-snikket-if-i-have-a-dynamic-ip-address).
{{< /panel >}}
...
Now, add an A record for your IP address on the domain you want to run SnikketX on. In the examples I'm going to use 'chat.example.com' as the domain,
and '203.0.113.123' as the IP address. This will be the primary domain for your SnikketX instance.
...
{{< panel style="info">}}
**Check that firewall!**
If you're setting up SnikketX at home, or behind a router or firewall, now is a good time to check that you have all the [required ports open
...
Docker is a handy tool for running self-contained services known as "containers". We use Docker to provide SnikketX
...
SnikketX is not compatible with the following host systems:
...
```bash
# The primary domain of your SnikketX instance
...
Earlier we mentioned that SnikketX needs access to the HTTP+HTTPS ports on the server. If you already
have websites or other web services on the server where you are installing SnikketX, **now is
the time to configure your reverse proxy** to share web traffic with your SnikketX instance.
...
The first time you run this command docker will download SnikketX. In a moment it should complete,
and SnikketX should be running and accessible via the web (e.g. `http://chat.example.com/`). As
...
and anyone else you want to join your SnikketX instance. It won't be empty for long!
