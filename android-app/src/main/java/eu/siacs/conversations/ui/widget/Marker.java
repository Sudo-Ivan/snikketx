...
public class Marker extends SimpleLocationOverlay {
     ...
     * @param position The geographic position where the marker will be drawn (if it is inside the
     *     view)
     */
    ...
    public void draw(final Canvas c, final MapView view, final boolean shadow) {
        ...
        view.getProjection()
                .toPixels(this.position == null ? view.getMapCenter() : position, mapPoint);

        c.drawBitmap(icon, mapPoint.x - icon.getWidth() / 2, mapPoint.y - icon.getHeight(), null);
    }
}
